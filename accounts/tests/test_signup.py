...
ROSTER = f"registration_number,full_name\n{REG},Ada Obi\nMOUAU/PSB/26/012346,\n"
...
class SignupTests(TestCase):
    def test_full_success_with_autoenroll_and_claim(self):
        ...
        c2 = make_course(
            code="PSB 414"
        )  # e.g. carry-over student listed by two lecturers
        import_roster(
            c2,
            io.StringIO(f"registration_number,full_name\n{REG},Ada Obi\n"),
            actor=None,
        )
        r = self.client.post(
            ...
            {
                "reg_no": f" {REG.lower()} ",
                "full_name": "ada obi",  # messy but matching
                "email": "ada.obi@example.com",
                "password1": "sensible-password-1",
                "password2": "sensible-password-1",
            },
        )
    ...
    def test_signup_throttle(self):
        ...
        self.assertRedirects(
            r, reverse("accounts:login"), fetch_redirect_response=False
        )
        self.assertEqual(User.objects.filter(global_role="student").count(), 0)
    ...
    def test_inactive_roster_number_cannot_sign_up(self):
        ...
        import_roster(
            c,
            io.StringIO("registration_number,full_name\nMOUAU/PSB/25/018001,Old Row\n"),
            actor=None,
        )
        import_roster(
            c, io.StringIO("registration_number,full_name\n"), actor=None
        )  # deactivate all
        r = self.client.post(
            reverse("accounts:signup"),
            {
